...
local function _normalizeRes(res)
    ...
    -- Same fix as whitelist.lua's _normalizeRes: don't accept a non-2xx
    -- response's error body (e.g. GitHub 404 page) as a valid fetch result.
...
for i = 1, CFG.maxRetries do
    -- cache-buster: raw.githubusercontent.com CDN-caches per exact URL, so a
    -- whitelist.lua you just pushed can come back stale/near-empty without this
    local _bustUrl = CFG.whitelistUrl .. "?_cb=" .. tostring(os.time())
    ok, src = safeGetTimeout(_bustUrl, CFG.timeout)
    if ok and src and #src > 32 then break end
    warn(("[ SpectreWare Gateway ]: fetch attempt %d/%d failed (bytes=%d)"):format(i, CFG.maxRetries, src and #src or 0))
