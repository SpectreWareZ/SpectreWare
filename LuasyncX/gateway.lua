...
local LOADER = (function()
    ...
    local built, api = pcall(function()
        ...
        -- ── Status icons (วาดจาก Frame ล้วน ๆ — ไม่พึ่ง font emoji) ──────────────
        -- กัน emoji/สัญลักษณ์ที่หลุดมาจาก whitelist.lua (เช่น "✓" ที่ _stripDeco
        -- ไม่ครอบคลุม) ไม่ให้โผล่เป็นตัวอักษรดิบบนการ์ด โหลด — แปลงเป็นไอคอนเวกเตอร์แทน
        ...
        local function seg(parent, x1, y1, x2, y2, th, color)
            ...
            local f = mk("Frame", {
                ...
                Size = UDim2.fromScale(len + th, th),
        ...
        local function idot(parent, cx, cy, d, color)
            local f = mk("Frame", {
                BackgroundColor3 = color, BorderSizePixel = 0,
                AnchorPoint = Vector2.new(0.5, 0.5),
        ...
        local function iring(parent, cx, cy, d, thPx, color)
            local f = mk("Frame", {
        ...
        local ICON_DRAW = {
            check = function(p, c) poly(p, { { 0.10, 0.54 }, { 0.40, 0.82 }, { 0.92, 0.20 } }, 0.16, c) end,
            x = function(p, c)
                seg(p, 0.18, 0.18, 0.82, 0.82, 0.16, c)
                seg(p, 0.82, 0.18, 0.18, 0.82, 0.16, c)
            end,
            bolt = function(p, c)
            ...
            warn = function(p, c)
                poly(p, { { 0.50, 0.08 }, { 0.93, 0.88 }, { 0.07, 0.88 } }, 0.12, c, true)
                seg(p, 0.50, 0.38, 0.50, 0.60, 0.12, c)
                idot(p, 0.50, 0.76, 0.13, c)
            end,
        ...
        -- token → icon name (ตรงกับที่ notiflib.lua ใช้ แต่ครอบคลุม "✓" ตัวบาง
        -- ที่ _stripDeco ใน whitelist.lua ไม่ได้กรองออกด้วย)
        local ICON_TOKENS = {
            { "\u{2714}", "check" }, { "\u{2713}", "check" }, { "\u{2705}", "check" },
            { "\u{2718}", "x" },     { "\u{274C}", "x" },
            { "\u{26A1}", "bolt" },
            { "\u{26A0}", "warn" },
        ...
        -- หา token แรกที่เจอ ตัดออกจากข้อความ (รวม variation selector ถ้ามี) แล้วคืน
        -- ข้อความที่เหลือ + ชื่อไอคอน (nil ถ้าไม่เจอ)
        ...
        local status = mk("TextLabel", {
            BackgroundTransparency = 1, LayoutOrder = 1, Size = UDim2.new(1, 0, 1, 0),
            Font = Enum.Font.GothamMedium, Text = "Starting...", TextSize = 13, TextColor3 = C.text,
            TextXAlignment = Enum.TextXAlignment.Left, TextTruncate = Enum.TextTruncate.AtEnd,
        }, statusRow)

        -- ช่องไอคอน: ซ่อนอยู่จนกว่าข้อความจะมี token ที่รู้จัก (ดู extractIcon ด้านบน)
        local msgIcon = mk("Frame", {
            BackgroundTransparency = 1, LayoutOrder = 2, Visible = false, Size = UDim2.fromOffset(14, 14),
        }, statusRow)
        local msgIconCanvas = mk("Frame", { BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1) }, msgIcon)
        ...
        local function setStatus(text)
            ...
            status.Text = cleanText ~= "" and cleanText or text
            local drawFn = iconName and ICON_DRAW[iconName]
            if drawFn then
                for _, ch in ipairs(msgIconCanvas:GetChildren()) do ch:Destroy() end
                pcall(drawFn, msgIconCanvas, ICON_COLORS[iconName] or C.text)
